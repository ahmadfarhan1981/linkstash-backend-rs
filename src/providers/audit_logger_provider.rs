...
pub struct AuditLoggerProvider {
    audit_store: Arc<AuditStore>,
}

impl AuditLoggerProvider {
...
mod tests {
    ...
    async fn create_test_audit_logger_provider() -> AuditLoggerProvider {
...
// Standalone functions for use by stores and other components
// These create temporary provider instances to maintain compatibility
// with existing code that expects standalone functions

/// Log a successful login event (standalone function)
pub async fn log_login_success(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    target_user_id: String,
) -> Result<(), InternalError> {
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_login_success(ctx, target_user_id).await
}

/// Log a failed login attempt (standalone function)
pub async fn log_login_failure(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    failure_reason: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_login_failure(ctx, failure_reason, attempted_username).await
}

/// Log JWT issuance (standalone function)
pub async fn log_jwt_issued(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    target_user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_jwt_issued(ctx, target_user_id, issued_jwt_id, expiration).await
}

/// Log JWT validation failure (standalone function)
pub async fn log_jwt_validation_failure(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    failure_reason: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_jwt_validation_failure(ctx, failure_reason).await
}

/// Log JWT tampering attempt (standalone function)
pub async fn log_jwt_tampered(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    full_jwt: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_jwt_tampered(ctx, full_jwt, failure_reason).await
}

/// Log refresh token issuance (standalone function)
pub async fn log_refresh_token_issued(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    target_user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_refresh_token_issued(ctx, target_user_id, jwt_id, token_id).await
}

/// Log refresh token revocation (standalone function)
pub async fn log_refresh_token_revoked(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    target_user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_refresh_token_revoked(ctx, target_user_id, token_id).await
}

/// Log all refresh tokens invalidated (standalone function)
pub async fn log_all_refresh_tokens_invalidated(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_all_refresh_tokens_invalidated(ctx, user_id, reason).await
}

/// Log token invalidation failure (standalone function)
pub async fn log_token_invalidation_failure(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_token_invalidation_failure(ctx, user_id, reason, error_message).await
}

/// Log refresh token validation failure (standalone function)
pub async fn log_refresh_token_validation_failure(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    token_hash: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_refresh_token_validation_failure(ctx, token_hash, failure_reason).await
}

/// Log bootstrap completion (standalone function)
pub async fn log_bootstrap_completed(
    audit_store: &Arc<AuditStore>,
    actor_user_id: String,
    ip_address: Option<String>,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_bootstrap_completed(actor_user_id, ip_address, owner_username, system_admin_count, role_admin_count).await
}

/// Log owner activation (standalone function)
pub async fn log_owner_activated(
    audit_store: &Arc<AuditStore>,
    actor_user_id: String,
    ip_address: Option<String>,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_owner_activated(actor_user_id, ip_address, activation_method).await
}

/// Log owner deactivation (standalone function)
pub async fn log_owner_deactivated(
    audit_store: &Arc<AuditStore>,
    actor_user_id: String,
    ip_address: Option<String>,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_owner_deactivated(actor_user_id, ip_address, activation_method).await
}

/// Log CLI session start (standalone function)
pub async fn log_cli_session_start(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    command_name: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_cli_session_start(ctx, command_name, args).await
}

/// Log CLI session end (standalone function)
pub async fn log_cli_session_end(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    command_name: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_cli_session_end(ctx, command_name, success, error_message).await
}

/// Log user creation (standalone function)
pub async fn log_user_created(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    user_id: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_user_created(ctx, user_id, username).await
}

/// Log privileges changed (standalone function)
pub async fn log_privileges_changed(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    user_id: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_privileges_changed(ctx, user_id, old_is_owner, new_is_owner, old_is_system_admin, new_is_system_admin, old_is_role_admin, new_is_role_admin).await
}

/// Log operation rolled back (standalone function)
pub async fn log_operation_rolled_back(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    operation_type: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_operation_rolled_back(ctx, operation_type, reason, affected_user_id).await
}

/// Log password changed (standalone function)
pub async fn log_password_changed(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    target_user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_password_changed(ctx, target_user_id).await
}

/// Log transaction started (standalone function)
pub async fn log_transaction_started(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    operation_type: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_transaction_started(ctx, operation_type).await
}

/// Log common password list downloaded (standalone function)
pub async fn log_common_password_list_downloaded(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    url: &str,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_common_password_list_downloaded(ctx, url, password_count).await
}

/// Log password change requirement cleared (standalone function)
pub async fn log_password_change_requirement_cleared(
    audit_store: &Arc<AuditStore>,
    ctx: &RequestContext,
    target_user_id: String,
    ...
    let provider = AuditLoggerProvider::new(audit_store.clone());
    provider.log_password_change_requirement_cleared(ctx, target_user_id).await
}