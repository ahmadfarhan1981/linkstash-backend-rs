...
use crate::{
    ...
    providers::{PasswordValidatorProvider, TokenProvider},
    stores::{AuditStore, CredentialStore, SystemConfigStore},
};
...
/// ```
pub async fn setup_test_stores() -> (DatabaseConnections, Arc<CredentialStore>, Arc<AuditStore>) {
...
pub async fn setup_test_auth_services() -> (
    ...
    let secret_manager = Arc::new(
        crate::config::SecretManager::init().expect("Failed to initialize test SecretManager"),
    );

    // Create token provider with SecretManager
    let token_provider = Arc::new(TokenProvider::new(secret_manager, audit_store.clone()));

    // Create system_config_store for AppData
    let system_config_store =
        Arc::new(SystemConfigStore::new(auth_db.clone(), audit_store.clone()));
    ...
    credential_store
        .add_user(
            &password_validator,
            "testuser".to_string(),
            "TestSecure-Pass-12345-UUID".to_string(),
        )
        .await
