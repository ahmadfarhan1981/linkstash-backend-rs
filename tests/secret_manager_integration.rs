...
use linkstash_backend::config::{SecretError, SecretManager};
...
fn test_application_startup_with_valid_secrets() {
    ...
    let _guard = common::EnvGuard::new(vec![
        "JWT_SECRET",
        "PASSWORD_PEPPER",
    ...
    assert!(
        result.is_ok(),
        "SecretManager should initialize successfully with valid secrets"
    );
    ...
    assert_eq!(
        secret_manager_clone.jwt_secret(),
        "this-is-a-valid-jwt-secret-with-32-characters"
    );
}
...
fn test_application_fails_gracefully_with_missing_jwt_secret() {
    ...
    assert!(
        result.is_err(),
        "SecretManager should fail when JWT_SECRET is missing"
    );
...
fn test_application_fails_gracefully_with_missing_password_pepper() {
    ...
    assert!(
        result.is_err(),
        "SecretManager should fail when PASSWORD_PEPPER is missing"
    );
...
fn test_application_fails_gracefully_with_invalid_jwt_secret_length() {
    ...
    assert!(
        result.is_err(),
        "SecretManager should fail when JWT_SECRET is too short"
    );
    ...
    match err {
        SecretError::InvalidLength {
            ...
            assert_eq!(
                err_msg,
                "Secret 'JWT_SECRET' must be at least 32 characters, got 9"
            );
        }
...
fn test_application_fails_gracefully_with_invalid_password_pepper_length() {
    ...
    assert!(
        result.is_err(),
        "SecretManager should fail when PASSWORD_PEPPER is too short"
    );
    ...
    match err {
        SecretError::InvalidLength {
            ...
            assert_eq!(
                err_msg,
                "Secret 'PASSWORD_PEPPER' must be at least 16 characters, got 5"
            );
        }
...
async fn test_token_service_integration_with_secret_manager() {
    ...
    let token_service = Arc::new(TokenProvider::new(secret_manager.clone(), audit_store));

    // Verify TokenProvider was created successfully
    let user_id = Uuid::new_v4();
    let ctx = linkstash_backend::types::internal::context::RequestContext::new();
    let jwt_result = token_service
        .generate_jwt(&ctx, &user_id, false, false, false, vec![], false)
        .await;

    assert!(
        jwt_result.is_ok(),
        "TokenProvider should generate JWT successfully"
    );
}
...
async fn test_jwt_generation_works_with_secrets_from_secret_manager() {
    ...
    assert_eq!(
        claims.sub,
        user_id.to_string(),
        "JWT should contain correct user_id"
    );
}
...
async fn test_multiple_token_services_can_share_secret_manager() {
    ...
    let token_service1 = Arc::new(TokenProvider::new(secret_manager.clone(), audit_store1));
    let token_service2 = Arc::new(TokenProvider::new(secret_manager.clone(), audit_store2));

    // Generate JWT with first service
    let user_id = Uuid::new_v4();
    let ctx = linkstash_backend::types::internal::context::RequestContext::new();
    let (jwt, _jwt_id) = token_service1
        .generate_jwt(&ctx, &user_id, false, false, false, vec![], false)
        .await
        .unwrap();
