...
Build incrementally with working endpoints at each step, adding database persistence immediately after each feature is functional. Start with a basic login endpoint with hardcoded credentials, add database persistence for users, then add JWT generation, then refresh tokens with their own persistence. This ensures each feature is fully functional with real data before moving to the next.

---

## Phase 1: Basic Login Endpoint (Hardcoded Credentials)

- [x] 1. Create minimal auth models and error types
  - [x] 1.1 Add basic dependencies to Cargo.toml
    - Add poem-openapi Object derive support (already have)
    - Add serde for serialization
    - _Requirements: 7.1_
    
  - [x] 1.2 Create LoginRequest and TokenResponse models
...
- [x] 2. Implement basic login endpoint with hardcoded validation
  - [x] 2.1 Create AuthApi struct with login endpoint
...
## Phase 2: Database Setup and User Authentication
...
- [x] 3. Set up database infrastructure
  ...
  - [x] 3.1 Add database dependencies to Cargo.toml
  ...
  - [x] 3.2 Create database migration for users table
  ...
  - [x] 3.3 Create User entity model
  ...
  - [x] 3.4 Initialize database in main.rs
...
- [x] 4. Implement CredentialStore with user operations
  - [x] 4.1 Create CredentialStore struct
  ...
  - [x] 4.2 Implement add_user method
  ...
  - [x] 4.3 Implement verify_credentials method
  ...
  - [x] 4.4 Seed test user on startup





    - Add test user in main.rs if not exists (username: "testuser", password: "testpass"), add a todo comment here because this is just temporary.
    - Log success/failure
    - _Requirements: 5.1_
    

  - [x] 4.5 Update login endpoint to use database
    ...
    - Keep placeholder tokens for now
    - _Requirements: 1.1, 1.2_
    
  - [x] 4.6 Manual test database-backed login
  ...
  - [x] 4.7 Write unit tests for CredentialStore
